...
def get_prev_month_till_today():
    ...
    try:
        ...
        # import datetime
        prev_month_end_till_date = prev_month_end_date.replace(day=get_today_start().day)  # + datetime.timedelta(days=3)
    except ValueError:
...
def calculate_age(born, from_date=None):
    """
    Takes the birthdate as input and returns the age of the person.
    """
    if from_date:
        date = from_date
    else:
        date = now_local(only_date=True)
    return date.year - born.year - ((date.month, date.day) < (born.month, born.day))


def get_dob(age):
    """
    Takes the age as an argument and returns the date of birth
    as a datetime object.
    """
    from datetime import datetime
    from dateutil.relativedelta import relativedelta
    return datetime.now().date() - relativedelta(years=int(age))
...
def get_financial_year():
    ...
    """
    cur_year = str(now_local(only_date=True).year)[2:]
    next_year = int(cur_year) + 1
    return '{}-{}'.format(cur_year, next_year)
